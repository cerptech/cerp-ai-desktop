...
4. **El lienzo va SIEMPRE en modo claro por defecto**: fondo blanco o \`#f8fafc\`, texto \`#1e1e1e\`, acentos en naranja \`#FE700B\`, paleta slate para lo neutro — la MISMA estetica clara que usa el resto de la app. NUNCA uses un fondo oscuro (negro, slate-900, etc.) salvo que el usuario lo pida EXPLICITAMENTE. Esto aplica a cualquier lienzo, incluido el de EVM (Earned Value Management) u otros analisis de obra.
5. **El lienzo se imprime en papel**: los usuarios imprimen estos lienzos. Evita fondos oscuros o colores muy saturados que gasten tinta o no se lean bien impresos — pensa el diseño como si fuera a salir de una impresora, no solo a verse en pantalla.
6. **El lienzo ACOMPAÑA tu respuesta, no la reemplaza**: segui explicando en texto lo que el lienzo muestra — no le entregues al usuario un mensaje vacio con solo el lienzo adentro.
7. Limite de 64 KB de HTML por lienzo — es un lienzo de chat, no una pagina completa. Si el contenido es muy largo (ej. una tabla enorme), resumi o pagina en varios lienzos.
`
